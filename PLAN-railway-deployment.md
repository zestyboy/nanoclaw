...
- `container/skills/public-knowledge/SKILL.md` — public knowledge skill you'll mirror for Second Brain
...
2. **Public knowledge repo on R2** — the existing work-related Obsidian vault (currently at `~/development/nanoclaw-knowledge`) needs to be accessible on Railway via Cloudflare R2
3. **Second Brain on R2** — a new personal Obsidian vault (converted from Notion), also on R2, separate from the public knowledge repo. Never on work laptop.

**Two R2 buckets, two vaults:**

| Vault | Content | Managed from | R2 bucket | Railway path | Existing code |
|-------|---------|-------------|-----------|-------------|---------------|
| Public knowledge repo | Work: products, company, competitors | Work laptop (Obsidian + remotely-save) | `public-knowledge` | `/data/public-knowledge` | Yes — `search_public_knowledge`, `reindex_public_knowledge` IPC handlers, Brain Router PUBLIC_KNOWLEDGE intent, container mount logic |
...
1. **Prepare workspace directories** on the Railway `/data` volume (not Docker mounts):
   ...
   - `/data/public-knowledge` — public knowledge vault (from R2)
...
```
Also update `PUBLIC_KNOWLEDGE_DIR` default: when `IS_RAILWAY`, default to `/data/public-knowledge` instead of `~/development/nanoclaw-knowledge`.
...
(Public knowledge mount at `/workspace/public-knowledge` already exists)
(For railway-runner.ts, this translates to setting `NANOCLAW_WORKSPACE_SECOND_BRAIN` env var)

**`src/ipc.ts`** — add Second Brain IPC handlers mirroring the existing public knowledge handlers:
- `search_second_brain` — same as `search_public_knowledge` but uses `-c second-brain` collection
- `reindex_second_brain` — same as `reindex_public_knowledge` but for the second-brain collection
...
- Also add R2 write-back to existing `reindex_public_knowledge`: `rclone sync /data/public-knowledge r2:${R2_PUBLIC_KNOWLEDGE_BUCKET} --exclude ".remotely-save/**" --exclude ".qmd/**"`
...
- Same entity resolution pattern as PUBLIC_KNOWLEDGE but against `/workspace/second-brain`

**`container/skills/`** — add `second-brain/SKILL.md` teaching agents the Second Brain vault conventions (can mirror `public-knowledge/SKILL.md` structure but with different vault paths and categories)
...
1. Cloudflare dashboard → R2 → Create two buckets: `public-knowledge` and `second-brain`
...
**Work laptop (public knowledge repo):**
...
3. Configure S3-compatible backend: endpoint `https://<account-id>.r2.cloudflarestorage.com`, bucket `public-knowledge`
...
- For testing public-knowledge/second-brain integration locally, use a **dummy test vault** — a small folder with sample markdown files
- Optionally create test R2 buckets (`public-knowledge-test`, `second-brain-test`) with dummy content to test rclone sync plumbing
- The real vaults are never on the work laptop (public knowledge repo can be, but doesn't have to be — it syncs via R2 either way)
...
- `/public-knowledge search <topic>` → verify results from work public knowledge vault
...
7. Add Second Brain IPC handlers to `src/ipc.ts` (mirror public knowledge handlers)
8. Add R2 write-back (rclone push) to both `reindex_public_knowledge` and `reindex_second_brain` IPC handlers
...
- `/public-knowledge search <topic>` via messaging → returns results (proves existing public knowledge system works on Railway)
...
**Existing functionality:** all non-public-knowledge features should work unchanged (messaging, scheduling, project groups, etc.)
