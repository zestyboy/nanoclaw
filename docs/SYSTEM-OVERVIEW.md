...
## 8. The Second Brain Migration

### Context

The user (Niven) is migrating from a Notion "Ultimate Brain v3.0" system to Obsidian. This is a comprehensive personal productivity system combining:

- **PARA** (Tiago Forte) for organizational taxonomy
- **GTD** (David Allen) for task processing workflow
- **Daily Planning** for execution rhythm

### What the Notion System Contains

**8 core collections:**
1. **Tasks** — GTD-style task management (status, priority, due dates, snooze, My Day flag, contexts, sub-tasks, recurring tasks)
2. **Notes** — Knowledge capture (journal, meeting, web clip, reference, etc.)
3. **Projects** — Outcome-oriented containers (Planned → Doing → Done, also "Ongoing" for maintenance)
4. **Tags** — PARA taxonomy (Area / Resource / Entity types, hierarchical nesting)
5. **Goals** — Aspirational objectives (Dream → Active → Achieved, with milestones)
6. **Milestones** — Measurable checkpoints within goals
7. **People** — Personal CRM (contacts, meetings, relationships)
8. **Work Sessions** — Time tracking ledger

**Key workflows:**
- **My Day** — Daily planning: flag tasks → execute → clear all flags
- **My Week** — Weekly review: clear inboxes, reflect, plan ahead
- **GTD Pipeline** — Inbox → Clarify → Organize (Do Next / Calendar / Delegated / Deferred / Someday) → Execute
- **Goal Hierarchy** — Goal → Milestone → Project → Task (strict, no shortcuts)

**Key patterns:**
- Single source of truth: items live in one place, surfaced via filtered views
- Archive, never delete
- Contextual views over folder hierarchies
- Progressive complexity (advanced features disabled by default)

### How It Maps to NanoClaw

The Second Brain vault in NanoClaw is **not** a direct 1:1 recreation of the Notion system. Instead:

- The **Obsidian vault** (on R2) stores all the personal knowledge, tasks, projects, goals, etc. as markdown files with YAML frontmatter
- **Obsidian Bases** (`.base` files) replace Notion's database views — they query frontmatter properties and create filtered/sorted table/card views
- The **Brain Router** provides the messaging interface to interact with it — storing thoughts, searching knowledge, kicking off project work
- **qmd** provides the search layer so agents can find information in the vault

### The Migration Plan (see `/Users/niven/development/notion-analysis/obsidian-migration-plan.md`)

8 phases:
1. **Audit & Normalize** — Clean imported Notion data, add `kind` properties, standardize frontmatter, reorganize folders
2. **Build Core Bases** — Create `.base` files for each collection (tasks, notes, projects, tags, goals, milestones, people, books)
3. **Build Templates** — Template files with proper frontmatter and embedded Base views
4. **Build Homepage** — Central MOC (Map of Content) with embedded views
5. **Configure QuickAdd** — One-click capture macros
6. **Configure Metadata Menu** — Typed property fields for nice editing
7. **Configure Templater** — Folder-based auto-templates
8. **Verification & Testing** — Validate everything works

### Storage Architecture for the Second Brain

**The vault lives on Cloudflare R2** (object storage bucket). This is the source of truth.

**Railway deployment** syncs from R2 on startup and every 5 minutes. Agents read/write to the local `/data/second-brain` directory, and changes sync back to R2.

**Local development** uses a **dummy vault** (not the real personal data) for testing the system's vault integration, search, and Brain Router behavior without exposing personal knowledge.

The R2 bucket setup:
- Bucket name: `second-brain` (alongside existing `public-knowledge` bucket)
- Synced via `rclone` with S3-compatible API
- Environment variables needed:
  ...
  R2_PUBLIC_KNOWLEDGE_BUCKET=public-knowledge
  ```
